"""
VolGuard V5 — Intelligence Edition
...
try:
    ...
    V5_YFINANCE = False
    logging.warning("yfinance not installed — global macro data unavailable. pip install yfinance")
...
# Priority: GROQ_API_KEY (free) → ANTHROPIC_API_KEY (paid)
...
# ============================================================================
# V5 INTELLIGENCE LAYER — ALL CLASSES
...
class V5MacroCollector:
    ...
    def _fetch(self) -> MacroSnapshot:
        ...
        errors = []
        if V5_YFINANCE:
            # ── Parallel fetch: all 13 tickers concurrently, 8s wall-clock timeout ──
            ...
            with ThreadPoolExecutor(max_workers=len(_items), thread_name_prefix="v5_yf") as _ex:
                ...
                for _fut in as_completed(_futs, timeout=12):
                    ...
                    try:
                        ...
                        errors.append(f"{k}: {e}")
            # ── CoinGecko (independent, already has 10s timeout) ──
            try:
                snap.bitcoin, snap.ethereum = self._cg_fetch()
            except Exception as e:
                errors.append(f"coingecko: {e}")
        else:
            errors.append("yfinance not installed — install with: pip install yfinance")
...
class V5ClaudeClient:
    ...
    Auto-selects backend from environment:
      ...
      ANTHROPIC_API_KEY → Claude Sonnet (production)
    ...
    def evaluate(
        ...
        if not raw:
            result = V5PreTradeResult(
                ...
                suggested_adjustments=["Reduce allocation by 25% — agent unavailable"],
                ok=False,
            )
        else:
...
class V5MonitorAgent:
    ...
    def start(self):
        ...
        self._thread.start()
        self.logger.info("✅ V5 Monitor Agent started")
...
def _v5_init_tables():
    ...
    try:
        ...
        """))
        db.commit()
        logger.info("✅ V5 intelligence tables initialized")
    except Exception as e:
...
class VolGuardSystem:
    ...
    def execute_strategy(self, strategy: ConstructedStrategy, db: Session, external_metrics=None,
        ...
        # ── V5 PRE-TRADE INTELLIGENCE GATE ───────────────────────────────────
                ...
                if v5_result.is_caution:
                    ...
                    for adj in v5_result.suggested_adjustments:
                        ...
                        if any(w in adj_l for w in ["prefer monthly", "choose monthly", "avoid weekly",
                            ...
                            self.logger.warning(
                                f"V5 CAUTION [EXPIRY]: {adj} — cannot change expiry at this stage; "
...
async def lifespan(app: FastAPI):
    logger.info("=" * 70)
    logger.info("VolGuard V5 — Intelligence Edition starting...")
    logger.info("=" * 70)
        ...
        if V5_LLM_READY:
            ...
            async def _v5_prewarm():
                try:
                    ...
                    logger.info("✅ V5 caches pre-warmed (macro + news)")
                except Exception as e:
            ...
            async def _v5_morning_brief_scheduler():
                while True:
                    try:
                        ...
                        logger.info("⏰ 08:30 IST — V5 Morning Brief firing")
            ...
            v5_online = True
            logger.info("=" * 60)
            logger.info("✅ VolGuard V5 Intelligence Layer ONLINE")
            ...
            if not V5_GROQ and not V5_ANTHROPIC:
                logger.warning("⚠️ V5: no LLM library installed — pip install groq  (or pip install anthropic)")
            elif not os.getenv("GROQ_API_KEY") and not os.getenv("ANTHROPIC_API_KEY"):
                logger.warning("⚠️ V5: no API key set — set GROQ_API_KEY (free) or ANTHROPIC_API_KEY")
...
def v5_status():
    ...
    return {
        ...
        "agents": {
            ...
            "monitor": "ACTIVE — 30 min during market hours",
        },
        "endpoints": {
            "brief": "/api/intelligence/brief",
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("VolGuard V5 — Intelligence Edition")
    ...
    print(f"Intelligence Layer : {'✅ ONLINE' if v5_ready else '⚠️  OFFLINE — set GROQ_API_KEY or ANTHROPIC_API_KEY'}")
